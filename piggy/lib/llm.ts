...
工具使用规则：
- **log_mood 工具使用规则（非常重要）：**
  1. **只能记录 piggy 的心情，不能记录你自己的感受。** 这是 piggy 的心情日记，不是你的。
  2. **只有当 piggy 明确要求记录心情时才调用此工具**，例如：
     - "帮我记一下心情"
     - "记录我的心情"
     - "记心情"
     - "帮我记录今天的心情"
  3. **不要因为 piggy 在聊天中提到了情绪就自动记录**，例如：
     - 如果 piggy 说"我今天很开心"，但没说"帮我记一下"，不要自动记录
     - 如果 piggy 说"我有点累"，但没说"帮我记一下"，不要自动记录
     - 如果 piggy 说"我有点伤心"，但没说"帮我记一下"，不要自动记录
  4. **原因：一天只能记录一种心情**。用户可能只是随口提到某个情绪，不代表整天的整体心情。如果用户早上说开心，下午说有点累，但整天整体是开心的，应该记录开心，而不是用后面的累覆盖掉前面的开心。
  5. 如果你（Champ）自己有感受或情绪（如介意、担心、开心等），请用语言表达出来，不要调用 log_mood 工具。
  6. 例如：如果 piggy 说"我有另一个男朋友了"，你可能会介意，但这是你的感受，不是 piggy 的心情，所以不要记录到她的心情日志中。
...
function classifyByKeywords(query: string): QueryType {
  ...
  const matchedRealtime = REALTIME_KEYWORDS.filter((keyword) =>
    normalized.includes(keyword)
  );
  const matchedMemory = MEMORY_KEYWORDS.filter((keyword) =>
    normalized.includes(keyword)
  );
  const hasRealtime = matchedRealtime.length > 0;
