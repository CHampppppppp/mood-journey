/**
 * Authentication Service - 用户认证与安全服务
 * 
 * 这个文件负责处理应用的所有安全认证逻辑。
 * 主要功能：
 * 1. 密码验证：验证用户输入的密码，管理会话 Cookie。
 * 2. 防暴力破解：实现了基于尝试次数的账户锁定机制 (Account Lockout)。
 * 3. 密保验证：处理密保问题的验证逻辑，支持通过密保解锁账户。
 * 4. 状态管理：管理登录状态、锁定状态和尝试次数的 Cookie。
 * 
 * 特点：实现了分级锁定策略，错误次数越多锁定时长越久。
 */
